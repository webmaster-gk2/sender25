...
class BounceMessage
  ...
  def raw_message
    ...
    mail.message_id = "<#{SecureRandom.uuid}@#{Sender25::Config.dns.return_path_domain}>"
    mail.to_s
  end
  ...
  def postmaster_address
    @server.postmaster_address || "postmaster@#{@message.domain&.name || Sender25::Config.sender25.web_hostname}"
  end
