...
module HasDNSChecks
  ...
  def check_spf_record
    ...
    if spf_records.empty?
      ...
      suitable_spf_records = spf_records.grep(/include:\s*#{Regexp.escape(Sender25::Config.dns.spf_include)}/)
      if suitable_spf_records.empty?
        self.spf_status = "Invalid"
        self.spf_error = "An SPF record exists but it doesn't include #{Sender25::Config.dns.spf_include}"
        false
      else
        self.spf_status = "OK"
        self.spf_error = nil
  ...
  def check_mx_records
    ...
    if records.empty?
      ...
      missing_records = Sender25::Config.dns.mx_records.dup - records.map { |r| r.to_s.downcase }
      if missing_records.empty?
        self.mx_status = "OK"
        self.mx_error = nil
      elsif missing_records.size == Sender25::Config.dns.mx_records.size
        self.mx_status = "Missing"
  ...
  def check_return_path_record
    ...
    elsif records.size == 1 && records.first == Sender25::Config.dns.return_path_domain
      self.return_path_status = "OK"
      self.return_path_error = nil
    else
      self.return_path_status = "Invalid"
      self.return_path_error = "There is a CNAME record at #{return_path_domain} but it points to #{records.first} which is incorrect. It should point to #{Sender25::Config.dns.return_path_domain}."
    end
  end
