...
class TrackDomain < ApplicationRecord
  ...
  def check_dns
    ...
    elsif records.size == 1 && records.first == Sender25::Config.dns.track_domain
      self.dns_status = "OK"
      self.dns_error = nil
    else
      self.dns_status = "Invalid"
      self.dns_error = "There is a CNAME record at #{full_name} but it points to #{records.first} which is incorrect. It should point to #{Sender25::Config.dns.track_domain}."
    end
