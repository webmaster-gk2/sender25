...
module Sender25
  ...
  RSpec.describe LegacyConfigSource do
    ...
    describe "the 'sender25' group" do
      it "returns a value for sender25.web_hostname" do
        expect(config.sender25.web_hostname).to eq "sender25.llamas.com"
      end

      it "returns a value for sender25.web_protocol" do
        expect(config.sender25.web_protocol).to eq "https"
      end

      it "returns a value for sender25.smtp_hostname" do
        expect(config.sender25.smtp_hostname).to eq "smtp.sender25.llamas.com"
      end

      it "returns a value for sender25.use_ip_pools?" do
        expect(config.sender25.use_ip_pools?).to eq false
      end

      it "returns a value for sender25.default_maximum_delivery_attempts" do
        expect(config.sender25.default_maximum_delivery_attempts).to eq 20
      end

      it "returns a value for sender25.default_maximum_hold_expiry_days" do
        expect(config.sender25.default_maximum_hold_expiry_days).to eq 10
      end

      it "returns a value for sender25.default_suppression_list_automatic_removal_days" do
        expect(config.sender25.default_suppression_list_automatic_removal_days).to eq 60
      end

      it "returns a value for sender25.use_local_ns_for_domain_verification?" do
        expect(config.sender25.use_local_ns_for_domain_verification?).to eq true
      end

      it "returns a value for sender25.default_spam_threshold" do
        expect(config.sender25.default_spam_threshold).to eq 10
      end

      it "returns a value for sender25.default_spam_failure_threshold" do
        expect(config.sender25.default_spam_failure_threshold).to eq 25
      end

      it "returns a value for sender25.use_resent_sender_header?" do
        expect(config.sender25.use_resent_sender_header?).to eq true
      end

      it "returns a value for sender25.smtp_relays" do
        expect(config.sender25.smtp_relays).to eq [
    ...
    describe "the 'main_db' group" do
      ...
      it "returns a value for main_db.username" do
        expect(config.main_db.username).to eq "sender25"
      end
      ...
      it "returns a value for main_db.database" do
        expect(config.main_db.database).to eq "sender25"
      end
    ...
    describe "the 'message_db' group" do
      ...
      it "returns a value for message_db.username" do
        expect(config.message_db.username).to eq "sender25"
      end
      ...
      it "returns a value for message_db.database_name_prefix" do
        expect(config.message_db.database_name_prefix).to eq "sender25"
      end
    end
    ...
    describe "the 'dns' group" do
      it "returns a value for dns.mx_records" do
        expect(config.dns.mx_records).to eq ["mx1.sender25.llamas.com", "mx2.sender25.llamas.com"]
      end

      it "returns a value for dns.spf_include" do
        expect(config.dns.spf_include).to eq "spf.sender25.llamas.com"
      end

      it "returns a value for dns.return_path_domain" do
        expect(config.dns.return_path_domain).to eq "rp.sender25.llamas.com"
      end

      it "returns a value for dns.route_domain" do
        expect(config.dns.route_domain).to eq "routes.sender25.llamas.com"
      end

      it "returns a value for dns.track_domain" do
        expect(config.dns.track_domain).to eq "track.sender25.llamas.com"
      end

      it "returns a value for dns.helo_hostname" do
        expect(config.dns.helo_hostname).to eq "helo.sender25.llamas.com"
      end

      it "returns a value for dns.dkim_identifier" do
        expect(config.dns.dkim_identifier).to eq "sender25"
      end

      it "returns a value for dns.domain_verify_prefix" do
        expect(config.dns.domain_verify_prefix).to eq "sender25-verification"
      end
    ...
    describe "the 'smtp' group" do
      ...
      it "returns a value for smtp.username" do
        expect(config.smtp.username).to eq "sender25server"
      end
      ...
      it "returns a value for smtp.from_name" do
        expect(config.smtp.from_name).to eq "Sender25"
      end

      it "returns a value for smtp.from_address" do
        expect(config.smtp.from_address).to eq "sender25@llamas.com"
      end
    end
