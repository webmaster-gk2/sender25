...
namespace :sender25 do
  ...
  task generate_config_docs: :environment do
    ...
    output = Konfig::Exporters::EnvVarsAsMarkdown.new(Sender25::ConfigSchema).export
    File.write("doc/config/environment-variables.md", output)

    output = Sender25::YamlConfigExporter.new(Sender25::ConfigSchema).export
    File.write("doc/config/yaml.yml", output)
  end

  desc "Generate Helm Environment Variables"
  task generate_helm_env_vars: :environment do
    puts Sender25::HelmConfigExporter.new(Sender25::ConfigSchema).export
  end
...
Rake::Task["db:migrate"].enhance do
  Rake::Task["sender25:migrate_message_databases"].invoke
end
