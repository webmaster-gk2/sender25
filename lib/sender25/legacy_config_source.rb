...
module Sender25
  class LegacyConfigSource < Konfig::Sources::Abstract
    ...
    MAPPING = {
      "sender25.web_hostname" => -> (c) { c.dig("web", "host") },
      "sender25.web_protocol" => -> (c) { c.dig("web", "protocol") },
      "sender25.smtp_hostname" => -> (c) { c.dig("dns", "smtp_server_hostname") },
      "sender25.use_ip_pools" => -> (c) { c.dig("general", "use_ip_pools") },
      "logging.sentry_dsn" => -> (c) { c.dig("general", "exception_url") },
      "sender25.default_maximum_delivery_attempts" => -> (c) { c.dig("general", "maximum_delivery_attempts") },
      "sender25.default_maximum_hold_expiry_days" => -> (c) { c.dig("general", "maximum_hold_expiry_days") },
      "sender25.default_suppression_list_automatic_removal_days" => -> (c) { c.dig("general", "suppression_list_removal_delay") },
      "sender25.use_local_ns_for_domain_verification" => -> (c) { c.dig("general", "use_local_ns_for_domains") },
      "sender25.default_spam_threshold" => -> (c) { c.dig("general", "default_spam_threshold") },
      "sender25.default_spam_failure_threshold" => -> (c) { c.dig("general", "default_spam_failure_threshold") },
      "sender25.use_resent_sender_header" => -> (c) { c.dig("general", "use_resent_sender_header") },
      ...
      "sender25.smtp_relays" => -> (c) { c["smtp_relays"]&.map { |r| "smtp://#{r['hostname']}:#{r['port']}?ssl_mode=#{r['ssl_mode']}" } },
