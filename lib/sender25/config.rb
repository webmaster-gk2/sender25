  ...
  class << self
    ...
    def initialize_config
      ...
      if File.file?(config_file_path)
        ...
        when 1
          unless silence_config_messages
            warn "WARNING: Using legacy config file format. Upgrade your sender25.yml to use"
            warn "version 2 of the Sender25 configuration or configure using environment"
            warn "variables. See https://sender25server.io/config-v2 for details."
          end
          sources << LegacyConfigSource.new(yaml)
        when 2
          sources << Konfig::Sources::YAML.new(config_file)
        else
          raise "Invalid version specified in Sender25 config file. Must be 1 or 2."
        end
    ...
    def host_with_protocol
      @host_with_protocol ||= "#{Config.sender25.web_protocol}://#{Config.sender25.web_hostname}"
    end
    ...
    def signer
      @signer ||= begin
        key = OpenSSL::PKey::RSA.new(File.read(Config.sender25.signing_key_path))
        Signer.new(key)
      end
    end
      ...
      @graylog_logging_destination ||= begin
        ...
        proc do |_logger, payload, group_ids|
          ...
          notifier.notify!(short_message: short_message, **{
            ...
            _version: Sender25.version.to_s,
