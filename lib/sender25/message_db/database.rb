    ...
    class Database
      ...
      def select(table, options = {})
        ...
        if options[:order]
          ...
          raise Sender25::Error, "Invalid direction #{options[:direction]}" unless %w[ASC DESC].include?(direction)
      ...
      #
      def database_name
        @database_name ||= "#{Sender25::Config.message_db.database_name_prefix}-server-#{@server_id}"
      end
