  ...
  module MessageDB
    class Migration
      ...
      def self.run(database, start_from: database.schema_version, silent: false)
        files = Dir[Rails.root.join("lib", "sender25", "message_db", "migrations", "*.rb")]
        ...
        files.each do |version, file|
          ...
          require "sender25/message_db/migrations/#{version.to_s.rjust(2, '0')}_#{file}"
          klass = Sender25::MessageDB::Migrations.const_get(klass_name)
