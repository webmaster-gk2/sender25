  ...
  module MessageDB
    class LiveStats
      ...
      def total(minutes, options = {})
        if minutes > 60
          raise Sender25::Error, "Live stats can only return data for the last 60 minutes."
        end

        options[:types] ||= [:incoming, :outgoing]
        raise Sender25::Error, "You must provide at least one type to return" if options[:types].empty?
