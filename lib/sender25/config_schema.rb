...
module Sender25
  ...
  #
  #     rake sender25:generate_config_docs

  ConfigSchema = Konfig::Schema.draw do
    group :sender25 do
      string :web_hostname do
        description "The hostname that the Sender25 web interface runs on"
        default "sender25.example.com"
      end

      string :web_protocol do
        description "The HTTP protocol to use for the Sender25 web interface"
        default "https"
      end

      string :smtp_hostname do
        description "The hostname that the Sender25 SMTP server runs on"
        default "sender25.example.com"
      end
      ...
      string :signing_key_path do
        ...
        transform { |v| Sender25.substitute_config_file_root(v) }
      end
      ...
      string :trusted_proxies do
        array
        description "An array of IP addresses to trust for proxying requests to Sender25 (in addition to localhost addresses)"
    ...
    group :logging do
      ...
      boolean :enabled do
        description "Enable the Sender25 logger to log to STDOUT"
        default true
      end
    ...
    group :dns do
      ...
      string :domain_verify_prefix do
        ...
        default "sender25-verification"
      end

      string :custom_return_path_prefix do
        description "The domain to use on external domains which points to the Sender25 return path domain"
        default "psrp"
      end
    ...
    group :smtp do
      ...
      string :from_name do
        description "The name to use as the from name outgoing emails from Sender25"
        default "Sender25"
      end

      string :from_address do
        description "The e-mail to use as the from address outgoing emails from Sender25"
        default "sender25@example.com"
      end
    end
  ...
  class << self
    ...
    def substitute_config_file_root(string)
      ...
      string.gsub(/\$config-file-root/i, File.dirname(Sender25.config_file_path))
    end
