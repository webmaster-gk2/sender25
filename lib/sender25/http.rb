...
module Sender25
  module HTTP
    ...
    def self.request(method, url, options = {})
      ...
      if options[:sign]
        request.add_field "X-Sender25-Signature-KID", Sender25.signer.jwk.kid
        request.add_field "X-Sender25-Signature", Sender25.signer.sha1_sign64(request.body.to_s)
        request.add_field "X-Sender25-Signature-256", Sender25.signer.sign64(request.body.to_s)
      end

      request["User-Agent"] = options[:user_agent] || "Sender25/#{Sender25.version}"
