    ...
    def send_message(message, force_rcpt_to = nil)
      ...
      begin
        if message.bounce
          ...
          # Sender25 - Using mail_from from message
        ...
        begin
          if @smtp_client.nil?
            ...
            # Sender25 - Added server id and message id to log
            log "-> #{message.server.id}::#{message.id}"
          ...
          # Sender25 - Added IOErrors to rescue
        ...
        # Sender25 - Added source address to details
        if @smtp_client.source_address
          result.details += " (from #{@smtp_client.source_address})"
        end
