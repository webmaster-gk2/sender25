  ...
  module SMTPServer
    class Client
      ...
      def initialize(ip_address)
        ...
        # Sender25 - Remove ::ffff: from the start of ipv4 addresses
        @ip_address = ip_address.gsub("::ffff:", "")
        if @ip_address
          check_ip_address
          @state = :welcome
        else
      ...
      # Sender25 - Additional log
      def log2(text)
        return false unless @logging_enabled
        Postal.logger_for(:rdns_error).debug "[#{id}] #{text}"
      end

      private

      # Sender25 - Added function to resolve hostname
      ...
      def proxy(data)
        if m = data.match(/\APROXY (.+) (.+) (.+) (.+) (.+)\z/)
          ...
          # Sender25 - Change from "Postal" to "Sender25"
          "220 #{Postal.config.dns.smtp_server_hostname} ESMTP Sender25/#{id}"
        else
          @finished = true
          "502 Proxy Error"
        end
      ...
      def ehlo(data)
        ...
        # Sender25 - New authentication rules
        unless check_ip_whitelist
          if @ip_address == @hostname
            log "\e[33m   WARN: No reverse DNS set #{@hostname}\e[0m"
            return "550 5.7.1 authentication checks. To best protect our users from spam, the 550 5.7.1 message has been blocked. Please visit 550 5.7.1"
          end
          if check_ip_blacklist
            log "\e[33m   WARN: Blocked by rules [EHLO]#{@helo_name} / [rDNS]#{@hostname} [#{@ip_address}] \e[0m"
            return "550 5.7.1 authentication checks. To best protect our users from spam, the 550 5.7.1 message has been blocked. Please visit 550 5.7.1"
          end
          unless prt_eq
            log "\e[33m   WARN: No consistent reverse DNS set [EHLO]#{@helo_name} / [rDNS]#{@hostname} [#{@ip_address}] \e[0m"
            log2 "No consistent reverse DNS set [EHLO]#{@helo_name} / [rDNS]#{@hostname} [#{@ip_address}]"
            # return "550 5.7.1 Administrative prohibition"
          end
        end
        transaction_reset
        @state = :welcomed
      ...
      def helo(data)
        ...
        unless check_ip_whitelist
          ...
          if check_ip_blacklist
            log "\e[33m   WARN: Blocked by rules [HELO]#{@helo_name} / [rDNS]#{@hostname} [#{@ip_address}] \e[0m"
            return "550 5.7.1 authentication checks. To best protect our users from spam, the 550 5.7.1 message has been blocked. Please visit 550 5.7.1"
          end
          unless prt_eq
            log "\e[33m   WARN: No consistent reverse DNS set [HELO]#{@helo_name} / [rDNS]#{@hostname} [#{@ip_address}] \e[0m"
            log2 "No consistent reverse DNS set [HELO]#{@helo_name} / [rDNS]#{@hostname} [#{@ip_address}]"
            # return "550 5.7.1 Administrative prohibition"
          end
        end
        transaction_reset
      ...
      # Sender25 - New authentication rules
      def prt_eq
        return true if check_ip_whitelist
      ...
      def check_ip_blacklist
        begin
          blacklist = Postal.config.smtp_server.blacklist_cidr.split(',')
          blacklist.each do |cidr|
            return true if IPAddr.new(cidr) == IPAddr.new(@ip_address)
          end
        rescue => e
          log "\e[33m   WARN: Error check ip #{e.class}: #{e.message}\e[0m"
        end
        false
      end
      ...
      def check_ip_whitelist
        begin
          whitelist = Postal.config.smtp_server.whitelist_cidr.split(',')
          whitelist.each do |cidr|
            return true if IPAddr.new(cidr).include? @ip_address
          end
        rescue => e
          log "\e[33m   WARN: Error reverse check ip #{e.class}: #{e.message}\e[0m"
        end
        false
      end
      ...
      def finished
        ...
        if @credential
          ...
          # Sender25 - New authentication rules
          authenticated_mail_from_domain = @credential.server.find_authenticated_domain_from_mail_from(@mail_from)
          if authenticated_domain.nil? && authenticated_mail_from_domain.nil?
            transaction_reset
            @state = :welcomed
          ...
          if authenticated_domain.nil?
            authenticated_domain = authenticated_mail_from_domain
          end
        end
